...
Cuando se analiza la siguiente tabla cruzada, que establece la relación entre el grupo de ingreso vs la corrupción, se observa que a medida que el grupo de ingreso es mejor, es decir de "upper middle income" en adelante, la frecuencia de corrupción menor a "3" en el ranking tiende a disminuir. Cabe recalcar que la primera fila de proporciones  corresponde a las proporciones a nivel de la fila, la segunda fila corresponde a las proporciones a nivel de columna y la tercera corresponde a las proporciones respecto al total.

```{r}

tablaCruzada <- CrossTable(life_expectancy$IncomeGroup, life_expectancy$Corruption, prop.r = TRUE, prop.c = TRUE, prop.chisq = FALSE)

tablaCruzadaFormato <-xtabs(~life_expectancy$IncomeGroup +life_expectancy$Corruption,data = df )
...
```{r}
# Suponiendo que tienes un data frame llamado 'data' con las columnas 'EsperanzaVida' y 'Region'
# Calcula la media de la esperanza de vida por región utilizando aggregate

result <- aggregate(life_expectancy$`Life Expectancy World Bank`, by = list(life_expectancy$Region), FUN = mean)

# Renombra las columnas para mayor claridad
colnames(result) <- c("Region", "Media_EsperanzaVida")
...
modelo_anova <- aov(life_expectancy$`Life Expectancy World Bank` ~ life_expectancy$Region, data = filtradoDeZonas)
...
# Prueba para conocer las diferencias: 

inferenciaDeDiferencia <-function(data, v1,v2, cme){
  
  x1 <- data %>%
  filter(data$Region == v1) %>%
  summarize(Media_EsperanzaVida = mean(`Life Expectancy World Bank`))
  ...
  intervalo_inf <- (x1 - x2) -  qt(1- 0.025, nrow(data)-3 ) * sqrt(cme*(1/n1 + 1/n2))
  intervalo_sup <- (x1 - x2) +  qt(1- 0.025, nrow(data)-3 )* sqrt(cme*(1/n1 + 1/n2))
  
  
  frase <- paste(v1," VS ", v2,": ", "[", intervalo_inf, ";", intervalo_sup, "]")
  print(frase)
  

  return(list(intervalo_inf,intervalo_sup)) 
  
}

v1 <- inferenciaDeDiferencia(life_expectancy,"Middle East & North Africa","East Asia & Pacific",25)

v2 <- inferenciaDeDiferencia(life_expectancy,"Middle East & North Africa","Europe & Central Asia",25)

v3 <- inferenciaDeDiferencia(life_expectancy,"Europe & Central Asia","East Asia & Pacific",25)
